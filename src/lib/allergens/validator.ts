 ...
 *
 * Les violations ne sont PAS déduplicées par participant (3 cœliaques = 3 violations
 * distinctes). L'UI est responsable de la consolidation de l'affichage.
 ...
 *
 * @example
 * const result = validatePlanning(planning, new Map(recettes.map(r => [r.id, r])), participants);
 * if (!result.valid) {
 *   // result.violations contient le détail de chaque problème
 * }
 */
