...
**TK-34 · S** — Unifier `check-dal-read-contract` (précis fonction→table) et `check-dal-reads` (large, tous accès + plannings.ts) en un seul checker AST précis-en-fonction/large-hors-fonction — ADR si tiré, pas acquis qu'un > deux.
...
| TK-34 | Unifier checkers DAL AST (TK-32/33) en un seul précis+large | P2 | S | À faire |
